...
ASR_1990$val <- round(ASR_1990$val,2) ###保留一位小数点
ASR_1990$lower <- round(ASR_1990$lower,2) ###保留一位小数点
ASR_1990$upper <- round(ASR_1990$upper,2) ###保留一位小数点
write.csv(ASR_1990,'ASR_1990 for incidence.csv')


####  map for ASR
worldData <- map_data('world')
country_asr <- ASR_1990
...
p <- ggplot()
total <- total %>% mutate(val2 = cut(val, breaks = c(0,10,20,40,80,160),
                                     labels = c("0.0~10.0","10.0~20.0","20.0~40.0",
                                                "40.0~80.0","80.0~100.0"),  ### breaks需要根据自己的实际结果来调整
...
p2 <- p + geom_polygon(data=total, 
  ...
  theme_void()+labs(x="", y="")+
  guides(fill = guide_legend(title='ASR for 1990(/10^5)'))+
...
ASR_2019$val <- round(ASR_2019$val,2) ###保留一位小数点
ASR_2019$lower <- round(ASR_2019$lower,2) ###保留一位小数点
ASR_2019$upper <- round(ASR_2019$upper,2) ###保留一位小数点
write.csv(ASR_2019,'ASR_2019 for incidence.csv')
...
p <- ggplot()
total <- total %>% mutate(val2 = cut(val, breaks = c(0,5,10,20,40,80,160),
                                     labels = c("0.0~5.0","5.0~10.0","10.0~20.0","20.0~40.0",
                                                "40.0~80.0","80.0+"),  ### breaks需要根据自己的实际结果来调整
                                     include.lowest = T,right = T))
...
p2 <- p + geom_polygon(data=total, 
  ...
  theme_void()+labs(x="", y="")+
  guides(fill = guide_legend(title='ASR for 2019(/10^5)'))+
  theme(legend.position = 'right')
p2
...
p2 <- p + geom_polygon(data=total, 
                       aes(x=long, y=lat, group = group,fill=val2),
                       colour="black",linewidth = .2) + 
  scale_fill_manual(values = c("#FFC312", "#C4E538", "#12CBC4", "#FDA7DF", "#ED4C67", "#CD3333", "#8B2323"))+
  theme_void()+labs(x="", y="")+
  guides(fill = guide_legend(title='Change in cancer cases'))+
  theme(legend.position = 'right')
p2

write.csv(country_asr,'change in cancer cases for incidence.csv')
...
worldData <- map_data('world')
total <- full_join(worldData,country_asr,by = c('region'='location'))
write.csv(country_asr,'EAPC for incidence.csv')

p <- ggplot()
p1 <- p + geom_polygon(data=total, 
                       aes(x=long, y=lat, group = group,fill=EAPC),
                       colour="black",linewidth = .2) +
scale_fill_gradient2(low = "#18dcff",mid = "#C4E538", high = "#EA2027",
                       midpoint = 0)+
...
ASR_1990$lower <- round(ASR_1990$lower,2) ###保留一位小数点
ASR_1990$upper <- round(ASR_1990$upper,2) ###保留一位小数点
write.csv(ASR_1990,'ASR_1990 for Deaths.csv')
...
p <- ggplot()
total <- total %>% mutate(val2 = cut(val, breaks = c(0,10,20,40,80,100),
                                     labels = c("0.0~10.0","10.0~20.0","20.0~40.0",
                                                "40.0~80.0","80.0+"),  ### breaks需要根据自己的实际结果来调整
                                     include.lowest = T,right = T))
...
ASR_2019$lower <- round(ASR_2019$lower,2) ###保留一位小数点
ASR_2019$upper <- round(ASR_2019$upper,2) ###保留一位小数点
write.csv(ASR_2019,'ASR_2019 for Deaths.csv')
...
case_2019 <- subset(EC,EC$year==2019 & 
                     ...
                     EC$metric== 'Number' &
                     EC$measure=='Deaths') ## 获取2019年EC发病数
case_1990 <- subset(EC,EC$year==1990 & 
...
p2 <- p + geom_polygon(data=total, 
  ...
  theme(legend.position = 'right')
p2
write.csv(country_asr,'change in cancer cases for death.csv')
...
worldData <- map_data('world')
total <- full_join(worldData,country_asr,by = c('region'='location'))
write.csv(country_asr,'EAPC for Deaths.csv')
