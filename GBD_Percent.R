...
library(ggplot2)
library(ggsci)
LC <- read.csv('BC_Percent.csv',header = T)  ## 读取我们的数据
...
case <- subset(LC,LC$year==2019 & 
                 ...
                 LC$metric== 'Number' &
                 LC$measure=='Deaths') ## 获取2019年EC年龄校正后发病率
...
LC_1990_2019$text <- as.character(round(LC_1990_2019$percent*100,2))  ### 添加标签变量，用于显示在图像中
#### 按照图中的顺序指定变量排列顺序
